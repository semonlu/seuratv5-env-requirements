...
Use this skill to prepare the runtime for the SeuratV5 course scripts before running any analysis module. The course code is R-first and Windows-oriented. It expects R 4.3 or newer, a matching Rtools version, JAGS, many CRAN/Bioconductor/GitHub R packages, several local tar.gz packages bundled in the course, and Python 3.10 or newer for reticulate/API/scverse-compatible workflows.

## Source Course

The default source is the local F: drive SeuratV5 course folder that contains:

- `1.*`: installers for R, RStudio, Rtools, and JAGS. The bundled course installer may be R 4.4.2, but the skill requirement is R 4.3 or newer.
...
1. Inspect whether Rscript is available and whether it points to R 4.3 or newer.
2. Add R, matching Rtools, JAGS, Git gzip, and Conda paths to the user PATH when needed.
...
- Add the detected R `bin` directory to the front of user PATH so `Rscript` resolves correctly.
- Add the matching Rtools paths when present, such as Rtools43 for R 4.3 or Rtools44 for R 4.4.
